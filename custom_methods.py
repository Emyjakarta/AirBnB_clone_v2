...
class CustomMethods:
    ...
    # value = line.getvalue().strip()
    # if not all(c in string.hexdigits for c in value):
    # raise ValueError('Input string is not a valid hexadecimal UUID')
    # return uuid.UUID(value)
        return uuid(line.getvalue().strip())
    # import uuid

    # def get_uuid(self, line):
        # Extract the UUID string from the line
        # uuid_string = line.getvalue().strip()
    ...
    def get_random_attribute(self):
        ...
        key = choice(
            list(self.__random_attributes.keys()))

        return key, choice(
            self.__random_attributes[key])
